...
class BuildServerOptions:
    ...
    remake_image = False
    image_name = "github_pages_build_image"
    image_version = "latest"
    url = "https://github.com/actions/jekyll-build-pages.git"
    branch = "v1.0.12"
    image_option_ruby_version = "3.1.6"
    container_name = "build_jekyll"
    wait_logs = 6
    clone_again = False
    ...
    def set_args(self, ap):
        ...
        self.volume_site = os.path.abspath(
            os.path.join(ap.root_dir, ap.volume_site))
        self.gemfile_path = os.path.abspath(
            os.path.join(ap.download_dir, ap.gemfile_path))
        self.dockerfile_path = os.path.abspath(
            os.path.join(self.download_dir, ap.dockerfile_name))
        self.entrypoint_name = os.path.abspath(
            os.path.join(self.download_dir, ap.entrypoint_name))
        self.remake_image = ap.remake_image
        self.image_name = ap.image_name
        self.image_version = ap.image_version
        self.url = ap.url
        self.branch = ap.branch
        self.clone_again = ap.clone_again
        self.image_option_ruby_version = ap.image_option_ruby_version
        self.container_name = ap.container_name
        self.wait_logs = ap.wait_logs

        if self.clone_again is True:
            self.remake_image = True
...
parser.add_argument("--url", type=str, default=options.url,
                    help="URL where jekyll-build-pages can be downloaded / Default=" + options.url)
parser.add_argument("--branch", type=str, default=options.branch,
                    help="Branch name of jekyll-build-pages / Default=" + options.branch)
parser.add_argument("--download_dir", type=str, default=options.download_dir,
                    help="Download directory / Default=" + options.download_dir)
...
parser.add_argument("--image_option_ruby_version", type=str,
                    default=options.image_option_ruby_version,
                    help="Ruby version / Default=" + options.image_option_ruby_version)
...
parser.add_argument("--dockerfile_name", type=str, default=options.dockerfile_path,
                    help="Dockerfile name / Default=" + options.dockerfile_path)
parser.add_argument("--entrypoint_name", type=str, default=options.entrypoint_name,
                    help="entrypoint name / Default=" + options.entrypoint_name)
...
# Settings: jekyll
parser.add_argument("--gemfile_path", type=str, default=options.gemfile_path,
                    help="Gemfile path / Default=" + options.gemfile_path)
parser.add_argument("--volume_site", type=str, default=options.volume_site,
                    help="Folder where build results are stored / Default=" + options.volume_site)
...
class SetupGithubPages:
    ...
    _opt = BuildServerOptions()

    def __init__(self, opt: BuildServerOptions):
        ...
        if self._opt.remake_image is True:
            if ret == 0:
    ...
    def download_jekyll_build_pages(self):
        ...
        if self._opt.flag_init is True:
            ...
            if os.path.exists(self._opt.download_dir):
                if self._opt.clone_again is True:
                    try:
                        if not os.access(self._opt.download_dir, os.W_OK):
                            os.chmod(self._opt.download_dir, 777)
                        os.remove(self._opt.download_dir)
                        print("  --> Remove directory: " + self._opt.download_dir)
                        _flag_download = True
                    except ImportError as e:
                        print("  --> There was a file that could not be operated on."
                              + self._opt.download_dir)
                        print(str(e))
                        _flag_download = False
                else:
                    print("  --> exists directory: " + self._opt.download_dir)
                    _flag_download = False
            ...
            if _flag_download is True:
                ...
                if ret == 0:
                    ...
                    ret, result = self.get_process(['git', 'clone', '--quiet',
                                                    '--no-progress', self._opt.url,
                                                    '--branch', self._opt.branch,
                                                    self._opt.download_dir])
                    if ret == 0:
                        print("  --> Get clone " + self._opt.url
                              + "(" + self._opt.branch + ")")
                    else:
                        print("  [ERROR] " + result)
    ...
    def build_docker_image(self):
        ...
        if self._opt.flag_init is True:
            ...
            print("  dir: " + self._opt.download_dir)
            ret, result = self.get_process(
                ['docker', 'images', '-q', self._opt.image_name + ':' + self._opt.image_version])
            if ret == 0 and result == "":
                os.chdir(self._opt.download_dir)
                ret = os.system('docker build'
                                # + ' --no-cache'
                                + ' --build-arg RUBY_VERSION=' + self._opt.image_option_ruby_version
            ...
            if flag_create is True:
                ...
                ret, _result = self.get_process(
                    ['docker', 'run', '-dit',
                     ...
                     # '--rm',
                     '-v', self._opt.gemfile_path + ":/root/src/Gemfile",
                     # '-v', self._entrypoint_name + ":/entrypoint.sh",
                     '-v', self._opt.src + ":/root/src",
                     '-v', self._opt.volume_site + ":/root/_site",
                ...
                if ret == 0:
                    ...
                    print("        site: " + self._opt.volume_site)
            else:
